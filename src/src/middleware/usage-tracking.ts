/**
 * Usage Tracking Middleware for CyberAgent
 *
...
function detectOperation(req: Request): string {
  ...
  // CyberAgent API operations
  if (path.includes('/jobs')) {
    if (method === 'POST') return 'job_create';
    if (method === 'DELETE') return 'job_cancel';
    if (path.includes('/status')) return 'job_status';
    return 'job_list';
  }
  if (path.includes('/scan')) return 'vulnerability_scan';
  if (path.includes('/pentest')) return 'penetration_test';
  if (path.includes('/malware')) return 'malware_analysis';
  if (path.includes('/exploit')) return 'exploit_execution';
  if (path.includes('/ioc')) return 'ioc_analysis';
  if (path.includes('/yara')) return 'yara_scan';
  if (path.includes('/target')) return 'target_operation';
  if (path.includes('/results')) return 'results_query';
  if (path.includes('/workflow')) return 'workflow_execution';
...
async function sendUsageReport(report: UsageReport, retryCount = 0): Promise<void> {
  ...
  try {
    ...
    if (response.status === 204 || response.ok) {
      logger.info('[Usage Tracking] Report sent', {
